...
| Location                                  | Kind             | Appears on home / archive? |
| ----------------------------------------- | ---------------- | -------------------------- |
| `content/posts/<section>/<slug>/index.md` | Post (sectioned) | Yes                        |
| `content/posts/<slug>/index.md`           | Post (orphan)    | Yes (home only)            |
| `content/<slug>/index.md`                 | Standalone page  | No                         |

Posts appear on the home page, posts index, section pages, and tag archives. Standalone pages are rendered but excluded from all listings.
...
- The filename starts with `_` (except `_index.md`, which is a listing metadata file, not a page)
- The file has no TOML frontmatter (`+++` delimiters)

## Page Bundles

A **page bundle** is a directory containing an `index.md` alongside related files. Bundles are the recommended way to organize pages because they keep content and assets together.
